import { Moon, Droplet, Zap, CalendarHeart } from "lucide-react";
...
export default function Patterns() {
  ...
  const hint =
    selectedDays.length === 0
      ? "tap any two days to compare"
      : selectedDays.length === 1
        ? "now pick a second day"
    ...
    <div className="patterns">
      <div className="patterns__inner">
        ...
        <div className="cycle-card">
          <div className="cycle-card__icon">
            <CalendarHeart size={18} />
          </div>
          <div className="cycle-card__content">
            <div className="cycle-card__headline">
              {CYCLE_CORRELATION.headline}
            </div>
            <p className="cycle-card__detail">{CYCLE_CORRELATION.detail}</p>
          </div>
          <div className="cycle-card__badge">{CYCLE_CORRELATION.dayRange}</div>
        </div>
        ...
        {selectedDays.length === 2 && compareA && compareB && (
          <div className="day-compare">
            <div className="day-compare__header">
              ...
              <button className="day-compare__clear" onClick={clearDays}>
                ✕ clear
              </button>
            </div>
            <div className="day-compare__grid">
              ...
              <div className="dc dc--head">
                Jun {compareA.day}
                {MIGRAINE_DAYS.includes(compareA.day) && (
                  <span className="dc__mig">migraine</span>
                )}
              </div>
              <div className="dc dc--head">
                Jun {compareB.day}
                {MIGRAINE_DAYS.includes(compareB.day) && (
                  <span className="dc__mig">migraine</span>
                )}
              </div>

              <div className="dc dc--label">HRV</div>
              <div className={`dc ${compareA.data.hrv < 40 ? "dc--warn" : ""}`}>
                {compareA.data.hrv} ms
              </div>
              <div className={`dc ${compareB.data.hrv < 40 ? "dc--warn" : ""}`}>
                {compareB.data.hrv} ms
              </div>

              <div className="dc dc--label">Temp</div>
              <div
                className={`dc ${compareA.data.temp > 37.0 ? "dc--warn" : ""}`}
              >
                {compareA.data.temp}°
              </div>
              <div
                className={`dc ${compareB.data.temp > 37.0 ? "dc--warn" : ""}`}
              >
                {compareB.data.temp}°
              </div>
              ...
              <div
                className={`dc ${compareA.data.stress === "High" ? "dc--warn" : ""}`}
              >
                {compareA.data.stress}
              </div>
              <div
                className={`dc ${compareB.data.stress === "High" ? "dc--warn" : ""}`}
              >
                {compareB.data.stress}
              </div>
        ...
        <div className="trigger-section">
          ...
          <div className="trigger-section__sub">
            Correlated trigger tracking matrix
          </div>
          {TRIGGER_ROWS.map(({ icon, name, pct }) => (
            <div key={name} className="trigger-row">
              ...
              <div className="trigger-row__bar-bg">
                <div
                  className="trigger-row__bar-fill"
                  style={{ width: `${pct}%` }}
                />
              </div>
